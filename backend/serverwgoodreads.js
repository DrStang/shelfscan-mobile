...
async function searchGoodreads(isbn, name, author){
    let url
...
// Helper function to merge book data from multiple sources
function mergeBookData(googleBook, openLibBook, goodreadsBook, goodreadsRating, originalTitle, originalAuthor) {
    // If we have no data from any source, return null
    if (!goodreadsBook && !googleBook && !openLibBook) return null;
    ...
    } else if (goodreadsRating?.rating > 0) {
    ...
    const primary = goodreadsBook || googleBook || openLibBook || {};
    const secondary = googleBook || openLibBook || goodreadsBook || {};
    ...
    try {
        ...
        const bookPromises = extractedBooks.map(async (book) => {
            try {
                ...
                const [googleBook, goodreadsBook, openLibBook] = await Promise.all([
                    searchGoogleBooks(book.title, book.author),
                    searchGoodreads(book.title, book.author),
                    searchOpenLibrary(book.title, book.author)
                ]);
                const isbn = goodreadsBook?.isbn || googleBook?.isbn || openLibBook?.isbn;
                const goodreadsRating = isbn ? await searchGoodreadsDB(isbn) : null;

                const mergedData = mergeBookData(googleBook, openLibBook, goodreadsBook, goodreadsRating, book.title, book.author);
