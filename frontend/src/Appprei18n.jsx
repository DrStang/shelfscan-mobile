...
import { Upload, WifiOff, Barcode, Book, Star, Loader2, Trash2, AlertCircle, X, Check, RotateCw, Camera, User, LogOut, ChevronRight, History, Globe, BookOpen, Key , Share as ShareIcon } from 'lucide-react';
...
function App() {
    ...
    const handleOnline = async () => {
      ...
      if (pending.length > 0) {
        ...
        const result = await processPendingScans(
            scanBooks,
            (current, total, scan) => {
              console.log(`Processing scan ${current}/${total}`);
            }
        );
        setSyncing(false);

        if (result.success) {
          alert(i18n.t('offline.syncComplete', { count: result.processed }));
        }
      }
    };
    const handleOffline = () => {
  ...
  const scanBooks = async (imageData = image) => {
    await Haptics.impact({style: ImpactStyle.Medium});

    if (isOffline) {
      const count = await queueScanForSync(imageData);
      setPendingScans(await getPendingScans());
      alert(i18n.t('offline.description'));
      return
    };
                            ...
                            {!image && (
                                <div className="flex gap-3 w-full max-w-md">
                                  ...
                                  <button
                                    ...
                                    className="flex-1 px-6 py-3 bg-green-600 text-white rounded-full font-semibold hover:bg-green-700 transition-colors flex items-center justify-center gap-2"
                                  >
                                    <Barcode className="w-5 h-5"/>
                                    {i18n.t('scan.scanBarcode')}
                                  </button>
